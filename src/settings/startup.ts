...
function onHeaderSectionClick(event: Event): void {
  ...
  document
    ...
    .forEach((el) =>
      el.id === tabButtonId
        ? el.classList.add("module-button-active")
        : el.classList.remove("module-button-active")
    );

  document
    .querySelectorAll<HTMLElement>(".module")
    .forEach(
      (el) => (el.style.display = el.id === tabPanelId ? "block" : "none")
    );
}
...
function createSectionHtml(module: ModuleInterface): HTMLElement {
  ...
  if (
    Object.keys(module.getDefaultConfig().template).length > 0 ||
    Object.keys(helpTemplate).length > 0
  ) {
    ...
    sectionEl.appendChild(
      createConfigWithHelp(module.getDefaultConfig().template, helpTemplate)
    );
    ...
    if (configString == null) {
      ...
      configEditorEl.rows = Object.keys(
        module.getDefaultConfig().template
      ).length;
    } else {
    ...
    configCurrentEl.replaceChildren(
      createConfigWithHelp(getModuleConfig(module), helpTemplate)
    );
  ...
  if (module instanceof ScheduledModuleInterface) {
    ...
    (module as PushModuleInterface).setCallback(
      (_, r) => (testOutputEl.textContent = resultToString(r))
    );
  ...
  if (module.getDefaultConfig().help) {
    ...
    configDescriptionEl.textContent =
      module.getDefaultConfig().help?.trim() ?? "";
...
function createConfigWithHelp(
  config: Record<string, string>,
  helpObj: Record<string, string>
): HTMLElement {
